...
var setLightState = function(light, body) {
    ...
    .catch(error => {
      ...
      setStateHelper(light, JSON.stringify(body))
      .then(() => {utils.logger(chalk.green(`Retry for ${light} successful!`)); resolve})
      .catch(error => {
...
var on = async function(room) {
  ...
  //   let lightType = await mongo.getLightType(light);
  ...
  //   try { await hue.setLightState(light, body) }
  //   catch (error) { failures.push(light) }
  // });
  ...
  for (light of lights) {
    let lightType = await mongo.getLightType(light);
    let body = (lightType === "Extended color light" && (Math.floor(Math.random()*100) % 4) === 0) ? new rest.Body(hourData.bri, null, true, null, hourData.xy) : new rest.Body(hourData.bri, ct, true);
    let failure = await setLightState(light, body);
...
var off = async function(room) {
    let failures = []; 
    let lights = await mongo.getLights(room);
    console.log(chalk.green(lights))

    lights.forEach(async light => {
      try { await hue.setLightState(light, {"on":false}); failures.push("hello")}
      catch(error) { failures.push(light); }
    });
    console.log(chalk.yellow(`Failures: ${failures}`));
    return(failures);
}
