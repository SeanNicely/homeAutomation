...
var find = function(collection, query, fields) {
	return new Promise((resolve,reject) => {
		mongo.connect(/*config.dbUrl*/"mongodb://127.0.0.1:27017/Hue", (err, db) => {
			if (err) reject(new Error("MongoAPI Find was unable to connect to database"));
			fields = fields || {};
			fields["_id"] = 0;
			try {
                ...
                reject(new Error("MongoAPI Find Error Accessing Data"));
            }
		});
	});
}

var findAll = function(collection, query, fields) {
    return new Promise((resolve, reject) => {
        mongo.connect("mongodb://127.0.0.1:27017/Mary", (err, db) => {
            if (err) reject(new Error("MongoAPI FindAll was unable to connect to database"));
            query = query || {};
            fields = fields || {};
            fields["_id"] = 0;
            try {
                db.collection(collection).find(query, fields).toArray((err, result) => {
                    if (err) reject(new Error("mongoApi.findAll says " + JSON.stringify(query) + ' not found in ' + collection))
                    resolve(result);
                    db.close();
                });
            } catch(err) {
                reject(new Error("MongoAPI FindAll Error Accessing Data"));
            }
        });
    });
}
